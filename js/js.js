    ...
    $.extend({
        jsMalditasCookies: function(opciones) {
            var configuracion = {
                ...
                mensaje: "Este sitio, como la mayoría, usa cookies. Si sigues navegando entendemos que acepta las política de uso.",
            ...
            if(Cookies.get(configuracion.cookie)!="aceptadas"){
                ...
                var enlace = contAviso.find("a.cookiesaceptar").on("click", function(e){
                    e.preventDefault();
                    cerrarAviso();
                    Cookies.set(configuracion.cookie, 'aceptadas', { expires: configuracion.expires }); // Expira en 3 años
                });
            }
            return this;
        }
